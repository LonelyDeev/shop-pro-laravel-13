...
class CreateTariffsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('tariffs', function (Blueprint $table) {
            ...
            // 1. ایندکس برای جستجوی تعرفه‌های یک روش ارسال
            $table->index('carrier_id');

            // 2. ایندکس برای فیلتر نوع تعرفه (داخل استان/خارج استان)
            $table->index('type');

            // 3. ایندکس ترکیبی برای تعرفه‌های یک روش ارسال با نوع خاص
            $table->index(['carrier_id', 'type']);

            // 4. ایندکس برای محدوده وزنی
            $table->index('max_weight');

            // 5. ایندکس برای مرتب‌سازی بر اساس هزینه
