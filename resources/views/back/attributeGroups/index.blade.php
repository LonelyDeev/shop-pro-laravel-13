                                                ...
                                                <tr id="attributeGroup-{{ $attributeGroup->id }}">
                                                    ...
                                                    <td class="text-center">
                                                        <div class="dropdown dropdown-action">
                                                            <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenu{{ $attributeGroup->id }}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                                <i class="fa-solid fa-ellipsis-vertical"></i>
                                                            </button>
                                                            <div class="dropdown-menu" aria-labelledby="dropdownMenu{{ $attributeGroup->id }}">
                                                                @can('attributes.index')
                                                                <a class="dropdown-item" href="{{ route('admin.attributes.index', ['attributeGroup' => $attributeGroup]) }}"><i class="fa-solid fa-arrow-down-short-wide mr-1"></i>مشاهده مقادیر</a>
                                                                <div class="dropdown-divider"></div>
                                                                @endcan
                                                                @can('attributes.groups.update')
                                                                <a class="dropdown-item" href="{{ route('admin.attributeGroups.edit', ['attributeGroup' => $attributeGroup]) }}"><i class="fa-solid fa-pencil mr-1"></i>ویرایش</a>
                                                                <div class="dropdown-divider"></div>
                                                                 @endcan
                                                                @can('attributes.groups.delete')
                                                                <button class="dropdown-item btn-delete" data-attributegroup="{{ $attributeGroup->id }}"  data-toggle="modal" data-target="#delete-modal"><i class="fa-solid fa-trash-can mr-1"></i> حذف</button>
                                                                @endcan
                                                            </div>
                                                        </div>
