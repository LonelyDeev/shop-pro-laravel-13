...
class ImportPostsJob implements ShouldQueue, ShouldBeUnique
{
    ...
    {
        return 'import_posts_' . ($this->jobData['user_id'] ?? 'guest');
    }
    ...
    public function handle()
    {
        try {
            ...
            $request->merge([
                ...
                'admin_id' => $this->jobData['user_id'],
                'update_duplicate' => $this->jobData['update_duplicate'],
            ]);

            $import = new PostsImport($request);
            ...
            if (Storage::disk('storage')->exists($this->filePath)) {
                ...
                Log::info('فایل اکسل مقالات پس از پردازش حذف شد', [
                    'file' => $this->filePath
                ]);
            }

            // ======== لاگ پردازش موفق ========
            $this->logImportSuccess('posts', $report, $this->filePath, $this->jobData['user_id'] ?? null);

            Log::info('واردات مقالات با موفقیت انجام شد', [
            ...
            Log::error('خطا در Job واردات مقالات: ' . $e->getMessage(), [
                'file' => $this->filePath,
    ...
    protected function storeErrorReport($report)
    {
        ...
        foreach (glob($logPath . '/posts_import_errors_*.json') as $file) {
            @unlink($file);
        }

        \Log::channel('import_errors')->error('خطاهای واردات مقالات', $report);

        $errorFile = storage_path('logs/posts_import_errors_' . date('Y-m-d_H-i-s') . '.json');
        file_put_contents($errorFile, json_encode($report, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));
    }

    public function failed(\Throwable $exception)
    {
        Log::error('واردات مقالات پس از ۱۰ بار تلاش ناموفق بود', [
            'user_id' => $this->jobData['user_id'] ?? null,
            'file' => $this->filePath,
            'error' => $exception->getMessage()
        ]);

        $this->logImportFailed('posts', $this->filePath, $this->jobData['user_id'] ?? null, $exception->getMessage());
