...
class ProductController extends Controller
{
    ...
    public function show_details(Product $product)
    {
        $this->authorize('products.details');
        ...
        // دریافت تمام گروه‌های ویژگی
        $attributeGroups = AttributeGroup::detectLang()->orderBy('ordering')->get();

        foreach ($attributeGroups as $group) {
            // دریافت ویژگی‌های این گروه برای محصول جاری
            $attributes = $product->get_attributes($group, null, false, []);

            if ($attributes && $attributes->count() > 0) {
                $groupedAttributes[$group->name] = $attributes;
            }
        }

        // اگر نیاز به ویژگی‌های با فیلتر قبلی دارید
        // مثال: دریافت ویژگی‌های بر اساس یک ویژگی قبلی
        $prevAttribute = null; // می‌توانید یک attribute مشخص را وارد کنید
        $groups = false; // اگر می‌خواهید گروه‌بندی انجام شود true کنید
        $attributesId = []; // آرایه‌ای از attribute_idها برای فیلتر

        $filteredAttributes = $product->get_attributes(
            $attributeGroups->first(), // گروه اول
            $prevAttribute,
            $groups,
            $attributesId
        );
        ...
        $salesStats = [
            ...
            'paid_orders'        => $product->orderItems()
                ->whereHas('order', fn($q) => $q->where('status','paid'))
                ->count(),
            'total_revenue'      => $product->orderItems()
                ->whereHas('order', fn($q) => $q->where('status','paid'))
                ->sum(DB::raw('quantity * price')),
            'today_orders'       => $product->orderItems()
                ->whereHas('order', fn($q) => $q->whereDate('created_at', today()))
                ->count(),
            'week_orders'        => $product->orderItems()
                ->whereHas('order', fn($q) => $q->whereBetween('created_at', [now()->startOfWeek(), now()->endOfWeek()]))
                ->count(),
            'month_orders'       => $product->orderItems()
                ->whereHas('order', fn($q) => $q->whereMonth('created_at', now()->month))
                ->count(),
            'total_stock'        => $product->prices()->sum('stock'),
        ...
        if (empty($metaTitle)) {
            $seoIssues[] = ['field' => 'meta_title', 'msg' => 'عنوان متا (Meta Title) تعریف نشده است.', 'fix' => 'یک عنوان متا بین ۵۰ تا ۶۰ کاراکتر اضافه کنید که کلیدواژه اصلی در ابتدای آن باشد.'];
        } elseif ($metaTitleLen < 30) {
            $seoWarnings[] = ['field' => 'meta_title', 'msg' => "عنوان متا خیلی کوتاه است ({$metaTitleLen} کاراکتر).", 'fix' => 'عنوان متا باید بین ۵۰ تا ۶۰ کاراکتر باشد.'];
        } elseif ($metaTitleLen > 65) {
            $seoWarnings[] = ['field' => 'meta_title', 'msg' => "عنوان متا خیلی بلند است ({$metaTitleLen} کاراکتر) و در نتایج گوگل قطع می‌شود.", 'fix' => 'عنوان را به کمتر از ۶۰ کاراکتر کاهش دهید.'];
        } else {
        ...
        if (empty($metaDesc)) {
            $seoIssues[] = ['field' => 'meta_description', 'msg' => 'توضیحات متا (Meta Description) تعریف نشده است.', 'fix' => 'یک توضیحات متا بین ۱۴۰ تا ۱۶۰ کاراکتر بنویسید که CTA داشته باشد.'];
        } elseif ($metaDescLen < 100) {
            $seoWarnings[] = ['field' => 'meta_description', 'msg' => "توضیحات متا خیلی کوتاه است ({$metaDescLen} کاراکتر).", 'fix' => 'توضیحات را به ۱۴۰ تا ۱۶۰ کاراکتر برسانید.'];
        } elseif ($metaDescLen > 165) {
            $seoWarnings[] = ['field' => 'meta_description', 'msg' => "توضیحات متا خیلی بلند است ({$metaDescLen} کاراکتر).", 'fix' => 'توضیحات را به کمتر از ۱۶۰ کاراکتر کاهش دهید.'];
        } else {
            $seoGood[] = "توضیحات متا مناسب ({$metaDescLen} کاراکتر)";
        }

        // 3. Alt تصویر
        if (empty($product->image_alt)) {
            $seoIssues[] = ['field' => 'image_alt', 'msg' => 'Alt تصویر اصلی خالی است.', 'fix' => 'متن Alt را با کلیدواژه اصلی پر کنید (مثلاً نام محصول + برند).'];
        } else {
            $seoGood[] = 'Alt تصویر اصلی تنظیم شده';
        }

        // 4. Slug
        $slug = $product->slug ?? '';
        if (empty($slug)) {
            $seoIssues[] = ['field' => 'slug', 'msg' => 'Slug (URL محصول) تعریف نشده است.', 'fix' => 'یک slug کوتاه، خوانا و حاوی کلیدواژه اصلی تعریف کنید.'];
        } elseif (mb_strlen($slug) > 75) {
            $seoWarnings[] = ['field' => 'slug', 'msg' => 'Slug خیلی بلند است و ممکن است بر SEO تأثیر منفی داشته باشد.', 'fix' => 'slug را به کمتر از ۷۵ کاراکتر کاهش دهید.'];
        } else {
            $seoGood[] = 'Slug تعریف شده';
        }

        // 5. توضیحات محصول (description)
        $description = strip_tags($product->description ?? '');
        $descLen = mb_strlen($description);
        if (empty($description)) {
            $seoIssues[] = ['field' => 'description', 'msg' => 'توضیحات محصول (description) خالی است.', 'fix' => 'حداقل ۳۰۰ کلمه محتوای منحصربه‌فرد با کلیدواژه‌های هدف بنویسید.'];
        } elseif ($descLen < 300) {
            $seoWarnings[] = ['field' => 'description', 'msg' => "توضیحات خیلی کوتاه است ({$descLen} کاراکتر — کمتر از ۳۰۰).", 'fix' => 'محتوا را افزایش دهید. محتوای بیشتر = رتبه بهتر.'];
        } else {
            $seoGood[] = "توضیحات محصول دارد ({$descLen} کاراکتر)";
        }

        // 6. بررسی تگ‌های HTML در description
        $rawDescription = $product->description ?? '';
        $missingTags = [];
        if (!empty($rawDescription)) {
            if (!preg_match('/<h[12]/i', $rawDescription))
                $missingTags[] = ['tag' => 'H1/H2', 'reason' => 'هدینگ اصلی برای ساختار محتوا و SEO ضروری است.'];
            if (!preg_match('/<h[34]/i', $rawDescription))
                $missingTags[] = ['tag' => 'H3/H4', 'reason' => 'زیرعنوان‌ها به تقسیم‌بندی محتوا کمک می‌کنند.'];
            if (!preg_match('/<ul|<ol/i', $rawDescription))
                $missingTags[] = ['tag' => 'UL / OL', 'reason' => 'لیست‌ها خوانایی را بالا می‌برند و Rich Snippet ایجاد می‌کنند.'];
            if (!preg_match('/<strong|<b>/i', $rawDescription))
                $missingTags[] = ['tag' => 'STRONG / B', 'reason' => 'برجسته‌سازی کلیدواژه‌ها به گوگل اهمیت آن‌ها را می‌رساند.'];
            if (!preg_match('/<table/i', $rawDescription))
                $missingTags[] = ['tag' => 'TABLE', 'reason' => 'جدول مشخصات فنی برای محصولات فیزیکی بسیار مفید است.'];
            if (!preg_match('/<img/i', $rawDescription))
                $missingTags[] = ['tag' => 'IMG', 'reason' => 'تصویر داخل محتوا تجربه کاربر و سئو را بهبود می‌دهد.'];
        }

        // 7. لینک‌های داخلی و خارجی در description
        $internalLinks = [];
        $externalLinks = [];
        if (!empty($rawDescription)) {
            preg_match_all('/<a[^>]+href=["\']([^"\']+)["\'][^>]*>(.*?)<\/a>/is', $rawDescription, $linkMatches);
            $appUrl = rtrim(config('app.url'), '/');
            foreach ($linkMatches[1] as $i => $href) {
                $linkText = strip_tags($linkMatches[2][$i]);
                if (str_starts_with($href, '/') || str_contains($href, $appUrl)) {
                    $internalLinks[] = ['url' => $href, 'text' => $linkText];
                } elseif (str_starts_with($href, 'http')) {
                    $externalLinks[] = ['url' => $href, 'text' => $linkText];
                }
            }
            if (count($internalLinks) === 0)
                $seoWarnings[] = ['field' => 'internal_links', 'msg' => 'هیچ لینک داخلی در توضیحات محصول وجود ندارد.', 'fix' => 'به صفحات مرتبط (دسته‌بندی، محصولات مشابه) لینک دهید.'];
            if (count($externalLinks) === 0)
                $seoWarnings[] = ['field' => 'external_links', 'msg' => 'لینک خارجی معتبر (مثل ویکی‌پدیا یا سازنده) وجود ندارد.', 'fix' => 'یک لینک خارجی به منابع معتبر اعتبار صفحه را نزد گوگل بالا می‌برد.'];
            else
                $seoGood[] = count($externalLinks) . ' لینک خارجی در محتوا';
        }

        // 8. Schema / Structured Data checklist
        $schemaChecks = [
            ['label' => 'name محصول',        'ok' => !empty($product->title)],
            ['label' => 'description',        'ok' => !empty($product->description)],
            ['label' => 'image محصول',        'ok' => !empty($product->image)],
            ['label' => 'brand',              'ok' => !empty($product->brand_id)],
            ['label' => 'sku / product_id',   'ok' => !empty($product->product_id)],
            ['label' => 'price (offers)',      'ok' => $product->prices->isNotEmpty()],
            ['label' => 'aggregateRating',    'ok' => $product->reviews_count > 0],
            ['label' => 'category',           'ok' => !empty($product->category_id)],
        ];

        // 9. عنوان انگلیسی
        if (empty($product->title_en))
            $seoWarnings[] = ['field' => 'title_en', 'msg' => 'عنوان انگلیسی ندارد.', 'fix' => 'عنوان انگلیسی برای sitemap و indexing چندزبانه مفید است.'];

        // 10. تاریخ انتشار
        if (empty($product->publish_date))
            $seoWarnings[] = ['field' => 'publish_date', 'msg' => 'تاریخ انتشار تعریف نشده (برای Schema datePublished).', 'fix' => 'تاریخ انتشار محصول را ثبت کنید.'];

        // امتیاز SEO
        $totalChecks = 10;
        $issueCount  = count($seoIssues);
        $warnCount   = count($seoWarnings);
...
/*        $adminName = auth()->user()->full_name ?? auth()->user()->name ?? 'مدیر';
        ...
        return view('back.products.details', compact(
            'product', 'salesStats', 'seoIssues', 'seoWarnings', 'seoGood',
            'missingTags', 'internalLinks', 'externalLinks', 'schemaChecks', 'seoScore',            'groupedAttributes',  // اضافه کردن ویژگی‌های گروه‌بندی شده
            'filteredAttributes'
        ));
    }
