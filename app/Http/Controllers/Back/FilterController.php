...
class FilterController extends Controller
{
    ...
    public function create()
    {
        ...
        $is_seller = Filter::isSeller()->exists();
    ...
    public function store(Request $request)
    {
        ...
        if ($request->is_seller && Filter::isSeller()->count() >= 1) {
            throw ValidationException::withMessages([
                'filter' => 'بیش از یک فیلتر فروشنده تعریف شده است'
            ]);
        }

        $filter = Filter::create([
            'title' => $request->title,
            'is_seller' => $request->is_seller ? true : false,
        ]);
        ...
        session()->put('toast-success','فیلتر با موفقیت ایجاد شد.');
    ...
    public function update(Filter $filter, Request $request)
    {
        ...
        if (!$filter->is_seller && $request->is_seller && Filter::isSeller()->count() >= 1) {
            throw ValidationException::withMessages([
                'filter' => 'بیش از یک فیلتر فروشنده تعریف شده است'
            ]);
        }
        ...
        session()->put('toast-success','فیلتر با موفقیت ویرایش شد.');
        return response("success");
    }
