$(document).ready(function () {
    ...
    $("#advantages").delegate(".js-icon-form-add", 'click', function (e) {
        ...
        if (advantageInput.val().trim().length > 0) {
            parent.append(
                ...
                '<input type="hidden" name="review[advantages][]" value="' + advantageInput
                    .val() + '">\n' +
                '</div>');
    ...
    $("#disadvantages").delegate(".js-icon-form-add", 'click', function (e) {
        ...
        if (disadvantageInput.val().trim().length > 0) {
            parent.append(
                ...
                '<button type="button" class="ui-dynamic-label-remove js-icon-form-remove"></button>\n' +
                '<input type="hidden" name="review[disadvantages][]" value="' +
                disadvantageInput.val() + '">\n' +
                '</div>');
    ...
    if ($("#countdown-seller-verify-end").length) {
        var $countdownOptionSellerEnd = $("#countdown-seller-verify-end");

        $countdownOptionSellerEnd.countdown({
            date: (new Date()).getTime() + 120 * 1000, // 1 minute later
            text: '<span class="day">%s</span><span class="hour">%s</span><span>: %s</span><span>%s</span>',
            end: function () {
                $countdownOptionSellerEnd.html('<a style="color:#00bfd6"  class="link-border-verify cursor-pointer form-account-link send-code-again-seller">ارسال مجدد</a>');

                $('.send-code-again-seller').click(function(e) {
                    e.preventDefault();
                    var mobile=$('#countdown-seller-verify-end').parents('form').find('input[name=mobile]').val();
                    $.ajax({
                        type: 'POST',
                        url: '/get-new-code-for-seller',
                        data: {
                            mobile:mobile
                        },
                        ...
                        beforeSend: function (xhr) {
                            ...
                            block('#seller-register-mobile');
                        },
                        complete: function () {
                            unblock('#seller-register-mobile');
                        }
                    });
                })
            }
    ...
    $(".product-additional-info .mask-handler").click(function (e) {
        e.preventDefault();
        var sumaryBox = $(this).parents('.is-masked ');
        sumaryBox.find('p').toggleClass('active');
        sumaryBox.find('.shadow-box').fadeToggle(0);
        $(this).find('.show-more').fadeToggle(0);
        $(this).find('.show-less').fadeToggle(0);
    });
    ...
    $(".product-carousel-story").owlCarousel({
        rtl: true,
        margin: 10,  // فاصله بین هر استوری
        nav: true,
        navText: ['<i class="fa fa-angle-right"></i>', '<i class="fa fa-angle-left"></i>'],
        dots: false,
        loop: false,  // اگر میخوای بینهایت نچرخه
        responsiveClass: true,
        responsive: {
            0: {
                items: 4,      // در موبایل: 2 تا کامل + نصفی برای القای اسکرول
                slideBy: 1
            },
            576: {
                items: 5,      // تبلت کوچک
    ...
    $(".posts-category-carousel").owlCarousel({
        rtl: true,
        margin: 10,
        nav: true,
        navText: ['<i class="fa fa-angle-right"></i>', '<i class="fa fa-angle-left"></i>'],
        ...
        autoplayTimeout: 3000,    // زمان بین اسلایدها (میلی‌ثانیه) - 3000 = 3 ثانیه
        autoplayHoverPause: true, // وقتی ماوس روی اسلایدر می‌ره، حرکت خودکار متوقف بشه
        loop: true,               // چرخش بی‌نهایت (بعد از آخرین اسلاید، دوباره از اول)
        responsiveClass: true,
        responsive: {
    ...
    $(".suggestions-post-carousel-one-block").owlCarousel({
        rtl: true,
        margin: 10,
        nav: true,
        navText: ['<i class="fa fa-angle-right"></i>', '<i class="fa fa-angle-left"></i>'],
...
$('#newsletter-form').on('submit', function(e) {
    ...
    submitBtn.text('در حال ثبت...').prop('disabled', true);
...
$(function () {
    ...
    function initSwipeCarousel($carousel) {
        // اگر این کاروسل روی صفحه وجود ندارد، رد شو
    ...
    // ---------- فعال‌سازی هر دو کاروسل ----------
    initSwipeCarousel($('#mainCarousel'));
    initSwipeCarousel($('#carouselExampleIndicators'));
});
