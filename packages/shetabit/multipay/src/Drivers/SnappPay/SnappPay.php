...
class SnappPay extends Driver
{
    ...
    public function purchase(): string
    {
        ...
        if (!is_null($email = $this->invoice->getDetail('email'))) {
            $data['email'] = $email;
        }

        if (!is_null($firstName = $this->invoice->getDetail('firstName'))) {
            $data['firstName'] = $firstName;
        }

        if (!is_null($lastName = $this->invoice->getDetail('lastName'))) {
        ...
        \Log::info('=== SNAPPAY CALLBACK URL DEBUG ===', [
            'settings_callbackUrl' => $this->settings->callbackUrl ?? null,
            'invoice_detail_returnURL' => method_exists($this->invoice, 'getDetail')
                ? $this->invoice->getDetail('returnURL')
                : null,
            'final_returnURL_sent_to_snapppay' => $data['returnURL'] ?? null,
        ]);



        \Log::info('SnappPay API Request (BEFORE SEND):', [
            'url' => $this->settings->apiPaymentUrl.self::TOKEN_URL,
            'data' => $data,
            'invoice_amount' => $this->invoice->getAmount(),
            'invoice_details' => $this->invoice->getDetails(),
        ]);
        ...
        \Log::info('SnappPay API Response:', [
        ...
        $response = $this
            ...
            ->post(
                ...
                [
                    ...
                    RequestOptions::HTTP_ERRORS => false,
                    RequestOptions::TIMEOUT => 30, //default 10 seconds
                ]
            );
    ...
    public function eligible()
    {
        ...
        $response = $this->client->get($this->settings->apiPaymentUrl.self::ELIGIBLE_URL, [
            ...
            RequestOptions::HTTP_ERRORS => false,
            RequestOptions::TIMEOUT => 30, // default was 10 seconds
        ]);
